    ...
    <AppShell role={profile.role}>
      <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <PageHeader
          ...
          subtitle="Atualize os dados cadastrais sem alterar as regras de estoque ou o histórico operacional."
          actions={(
            <Link
        ...
        <div className="mt-4 flex items-center gap-2 text-sm text-[var(--color-on-surface-variant)]">
          ...
          <StatusBadge tone={product.active ? "success" : "neutral"}>{product.active ? "Ativo" : "Inativo"}</StatusBadge>
        </div>
        ...
        <DataCard className="mt-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-semibold">Disponibilidade do produto</h2>
              <p className="mt-1 text-sm text-[var(--color-on-surface-variant)]">
                Inativar preserva o cadastro e impede sua exibição normal para operadores.
              </p>
            </div>
