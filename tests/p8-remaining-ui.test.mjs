...
test("P8.3c inventory adopts AppShell and semantic inventory primitives", async () => {
  ...
  for (const token of ["AppShell", "PageHeader", "DataCard", "StatusBadge"]) {
...
test("P8.4 movement page and form adopt Design System without changing transaction contract", async () => {
  const page = await read(paths.movementPage);
  const form = await read(paths.movementForm);

  for (const token of ["AppShell", "PageHeader", "DataCard"]) {
    assert.match(page, new RegExp(token));
  }
  assert.match(page, /<AppShell\s+role=\{profile\.role\}/);
  assert.match(form, /import\s+\{\s*Button\s*\}/);
  assert.match(form, /import\s+\{\s*DataCard\s*\}/);
  assert.match(form, /action=\{registerMovementAction\}/);
  for (const field of ["operation_id", "type", "product_id", "quantity"]) {
    assert.match(form, new RegExp(`name=["']${field}["']`));
  }
  assert.match(form, /crypto\.randomUUID/);
  assert.match(form, /window\.confirm/);
  assert.match(form, /INITIAL/);
  assert.match(form, /role="alert"/);
  assert.match(form, /aria-live="polite"/);
  assert.match(form, /font-data/);
});
...
test("P8.4 history adopts shell/primitives and remains read-only with approved filters", async () => {
  ...
  assert.match(source, /<AppShell\s+role=\{profile\.role\}/);
  for (const field of ["product", "type", "actor", "from", "to"]) {
...
test("P8.5 admin users adopts Design System and preserves privileged profile action", async () => {
  ...
  for (const token of ["AppShell", "PageHeader", "DataCard", "StatusBadge", "Button"]) {
    assert.match(source, new RegExp(token));
  }
  assert.match(source, /<AppShell\s+role=\{currentProfile\.role\}/);
  assert.match(source, /action=\{updateProfileAction\}/);
  for (const field of ["user_id", "role", "active"]) {
    assert.match(source, new RegExp(`name=["']${field}["']`));
  }
  assert.match(source, /currentProfile\.role\s*!==\s*"ADMIN"/);
...
test("P8.5 adjustment adopts transactional Design System without changing adjustment action", async () => {
  const page = await read(paths.adjustmentPage);
  const form = await read(paths.adjustmentForm);
  ...
  assert.match(form, /import\s+\{\s*Button\s*\}/);
  assert.match(form, /import\s+\{\s*DataCard\s*\}/);
  assert.match(form, /action=\{registerAdjustmentAction\}/);
  for (const field of ["operation_id", "product_id", "quantity", "reason"]) {
    assert.match(form, new RegExp(`name=["']${field}["']`));
  }
  assert.match(form, /crypto\.randomUUID/);
  assert.match(form, /window\.confirm/);
...
test("P8.6 all authenticated operational/admin pages use AppShell while public auth stays public", async () => {
  const authenticated = [
    ...
    ...Object.values(paths).filter((path) => !path.endsWith("-form.tsx") && !path.endsWith("movement-form.tsx") && !path.endsWith("adjustment-form.tsx")),
  ];

  for (const path of authenticated) {
    const source = await read(path);
    assert.match(source, /AppShell/, `${path} must use AppShell`);
  }

  for (const path of ["src/app/login/page.tsx", "src/app/register/page.tsx"]) {
    const source = await read(path);
    assert.doesNotMatch(source, /AppShell/, `${path} must remain outside authenticated shell`);
  }
});
...
test("P8.6 migrated remaining routes do not use legacy black CTA styling", async () => {
  for (const path of Object.values(paths)) {
    ...
    assert.doesNotMatch(source, /text-neutral-600/);
  }
});
