...
test('navegação móvel preserva a compactação qualificada pela PHASE-11', async () => {
  const mobile = await readFile(path.join(root, 'src/components/shell/MobileBottomNav.tsx'), 'utf8');
  const edit = await readFile(path.join(root, 'src/app/suppliers/[id]/edit/page.tsx'), 'utf8');

  assert.doesNotMatch(mobile, /min-w-24/);
  const minWidth = mobile.match(/min-w-\[(\d+)px\]/);
  assert.ok(minWidth, 'mobile navigation must declare an explicit compact minimum width');
  assert.ok(Number(minWidth[1]) <= 52, 'later phases may shrink the P11 minimum width but must not regress above 52px');
