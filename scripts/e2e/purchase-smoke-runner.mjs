...
import { runPurchaseQaFlow } from "./purchase-smoke-flow.mjs";
...
try {
  const purchase = await runPurchaseQaFlow({
    browser,
    baseURL,
    authStatePath,
    outputDir,
  ...
  if (!purchase.purchasePath) {
    recordRoute(report, {
      ...
      findings: [{
        ...
        severity: "error",
        detail: "QA purchase detail path was not available.",
      }],
    });
  }
